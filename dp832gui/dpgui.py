...
class DP832GUI(QMainWindow):
    ...
    def __init__(self):
        ...
        if constr is None: constr = "USB0::0x1AB1::0x0E11::DP8xxxxxxxxxx::INSTR"
    ...
    def addGraphs(self, numgraphs):
        ...
        for i in range(0,self.numchannels):
            ...
            cbChannel = QComboBox()
            cbChannel.addItem("CH1")
            cbChannel.addItem("CH2")
            cbChannel.addItem("CH3")
            cbChannel.setCurrentIndex(i)
            cbChannel.currentIndexChanged.connect(lambda x :self.setChannel(i, "CH%d"%(x+1)))
            
            lsettings = QHBoxLayout()
            lsettings.addWidget(QLabel("Points"))
            lsettings.addWidget(sbPoints)
            lsettings.addWidget(QLabel("Channel"))
            lsettings.addWidget(cbChannel)
